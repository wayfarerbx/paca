...
Add or edit a `SKILL.md` (or `paca.md`) under `services/ai-agent/src/skills/`. No code change is needed — `load_default_skills()` re-scans the directory on the next process start. Keep names consistent with the `trigger-*` prefix reserved for the fixed trigger-context skills in `trigger_skills.py`; `AgentContext` raises a hard error on any duplicate skill name.
