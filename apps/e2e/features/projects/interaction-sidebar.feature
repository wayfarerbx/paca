...
Feature: Sidebar interaction navigation
  ...
  entries inside an "Interactions" collapsible section.  Users never need to
  visit the Interactions settings page just to navigate to their work.
  Inside the "Interactions" section the entries are ordered: Timeline first,
  then Product Backlog, then sprint entries (active before planned).  Sprint
  entries are only visible when the Interactions section is in its expanded
  ...
  Rule: Displaying open interactions in the project sidebar
    ...
    Scenario: Timeline always appears as the first fixed entry in the Interactions section
    ...
    Scenario: Sidebar shows an "Interactions" section label grouping all interaction entries
      Then the sidebar should show a collapsible section labelled "Interactions"
      And the entries for timeline, product backlog, and sprints should appear beneath that label
      And collapsing the "Interactions" section should hide all interaction entries
